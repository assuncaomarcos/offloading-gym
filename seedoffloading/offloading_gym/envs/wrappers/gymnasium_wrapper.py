...
NOTE: This module is from TF-Agents master repository on Github because,
as of writing, the latest TF-Agents version available on pypi.org does not
include the wrapper.
"""
...
def spec_from_gym_space(
    ...
    if isinstance(space, gym.spaces.Discrete):
        ...
        dtype = np.integer
        maximum = try_simplify_array_to_value(np.asarray(space.nvec - 1, dtype=dtype))
        return specs.BoundedArraySpec(
        ...
        return tuple(
            [nested_spec(s, f"tuple_{i:d}") for i, s in enumerate(space.spaces)]
        )
        ...
        raise ValueError(
            f"The gymnasium space {space} is currently not supported."
        )
...
class GymnasiumWrapper(py_environment.PyEnvironment):
    ...
    def __getattr__(self, name: Text) -> Any:
        ...
        gym_env = super().__getattribute__("_gym_env")
